...
import pymysql.cursors  #Für Mysql

debug = False


def read(file, check):  #Parst die angegebene Datei und prüft auf plausible Werte
    data_file = open(file)
    data = None
    while not data:
        try:
...
def writedatabase(credentials, db_temp, db_hum): #Schreibt in die Datenbank (credentials = Dictionary,  db_temp & db_hum = float)
    ...
    try:
        ...
        connection.commit()
        if debug:
            print("Datenbank Verbindung I.O.")
    finally:
        connection.close()
