<?php
	...
	if ( isset( $_GET['function'] ) ){
		...
		switch($function){
			...
			case "showNew":
				//$query = mysqli_query($conn, "select temps.tstamp, temps.value as temp, hums.value as hum from temps, hums where temps.tstamp < current_timestamp and temps.tstamp = hums.tstamp order by temps.tstamp desc limit 1");

				$query = mysqli_query($conn, "select temps.tstamp, temps.temperature as temp, hums.humidity as hum from temps, hums where temps.tstamp < current_timestamp and temps.tstamp = hums.tstamp order by temps.tstamp desc limit 1");

				getJSONArray($query, "showNew");
				break;
			case "averagesDays":
